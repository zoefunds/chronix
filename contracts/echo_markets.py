...
"""
EchoMarkets Intelligent Contract
=================================

Project: EchoMarkets — a prediction-market platform where users bet on how
history will remember an event, adjudicated after long time horizons
(3 / 5 / 10 years, or permanent) by a GenLayer Intelligent Contract.

Runtime: GenLayer Studio / StudioNet. Gas token: GEN.
...
1. "PURE LOGIC" section — plain Python functions with NO `gl.*` calls and
   NO storage access. They take primitives in, return primitives out.
   These are the functions a later test-writing agent should target with
   pytest, because GenVM (`gl.Contract`, `gl.public.*`, `gl.message`, the
   nondeterministic web/LLM primitives) cannot be exercised from a normal
   pytest process — it only runs inside the GenVM sandbox / GenLayer
   Studio / genlayer's `gltest` harness. Keeping the math and state-machine
   rules gl-free means they can be unit tested with zero mocking.

2. "CONTRACT" section — the actual `gl.Contract` subclass. Every method
   here is a thin adapter: read inputs from `gl.message` / storage, hand
   off to a pure helper for any nontrivial decision, then write the result
   back to storage (and, for money, call the single `_send_gen` chokepoint
   last). This keeps the security-critical invariants (zero-before-transfer
   ordering, deadline enforcement, state-machine legality) easy to audit
   because they live in one place each, not scattered across methods.
...
This contract was written against:
  - https://docs.genlayer.com/developers/intelligent-contracts/introduction
    (confirmed: `@gl.public.view`, `@gl.public.write`,
    `@gl.public.write.payable`, `class X(gl.Contract)`, `from genlayer import *`)
  - https://docs.genlayer.com/developers/intelligent-contracts/storage
    (confirmed: typed class-body storage fields, `TreeMap[K, V]`,
    `DynArray[T]`, `@allow_storage` + `@dataclass` for custom structs,
    `int` is disallowed — use `u256`/`u32`/etc., `TreeMap.get_or_insert_default`)
  - https://docs.genlayer.com/developers/intelligent-contracts/equivalence-principle
    (confirmed: nondeterministic leader/validator execution, `gl.vm.Return`,
    `gl.vm.UserError`, `gl.vm.VMError` result variants, comparative /
    non-comparative / numeric-tolerance validator patterns, the explicit
    warning that "a validator that only checks the leader's calldata for
    valid JSON shape ... is not performing consensus")
  - A real contract generated locally by the official `genlayer` CLI
    (`genlayer new`, template `contracts/football_bets.py`, CLI v0.39.2),
    used as ground truth for exact runtime call spellings that the docs
    site's crawlable pages did not spell out precisely:
      `gl.message.sender_address`, `gl.get_webpage(url, mode="text")`,
      `gl.exec_prompt(prompt)`, `gl.eq_principle_strict_eq(fn)`,
      the `# { "Depends": "py-genlayer:test" }` header pragma, and the
      `TreeMap[...].get_or_insert_default(key)` accessor.

  Where the crawlable docs pages 404'd or did not show a full worked
  example (e.g. a dedicated "types" reference page, `gl.message.value`
  on a payable method, `@gl.evm.contract_interface` for native value
  transfer, and a deterministic on-chain clock primor timestamp source),
  this file falls back EXACTLY to the well-known, documented GenVM pattern
  described in the task brief:
    - payable writes read `gl.message.value`
    - `TreeMap[str, str]` (and typed dataclass TreeMaps) for state
    - `gl.vm.UserError` for validation errors
    - a single `_send_gen` emission helper built on
      `@gl.evm.contract_interface` + `_Recipient(Address(to)).emit_transfer(value=amount)`
    - `u256(...)` casts at every money boundary
  These are noted inline where used, not as unresolved TODOs — they are
  intentional, documented fallback choices, not guesses.

  For contract-visible time (needed to enforce `resolves_at` deadlines
  ON-CHAIN, not in a backend cron — see Section 2 below), no dedicated
  deterministic block-timestamp primitive was confirmed in the crawlable
  docs within scope. Per the task brief's explicit fallback instruction,
  this contract uses a caller-cannot-forge, nondet-cross-validated real
  time source: `request_adjudication` performs a nondeterministic fetch
  of a UTC time API and requires leader/validator agreement within a
  small tolerance window (mirroring the documented "numeric tolerance"
  validator pattern from the equivalence-principle page, applied to a
  timestamp instead of a price). This is deliberately over-conservative:
  it only ever REJECTS an early call, it never trusts a caller-supplied
  timestamp.
"""
